...
document.addEventListener('DOMContentLoaded', () => {
    const currentPath = window.location.pathname; // vd: "/feed", "/profile", "/chat"

    // Xóa tất cả active cũ trước
    document.querySelectorAll('.nav-icons-group a i, .profile-top').forEach(el => {
        el.classList.remove('active');
    });

    // Tìm link khớp với pathname hiện tại và set active
    document.querySelectorAll('.nav-icons-group a[data-path], .profile-top[data-path]').forEach(link => {
    ...
    // Xử lý riêng cho bell (không có <a> bao ngoài)
    const bellIcon = document.querySelector('.nav-icons-group .fa-bell[data-path]');
    if (bellIcon && currentPath.startsWith(bellIcon.getAttribute('data-path'))) {
    ...
    downloadIcon.addEventListener("click", () => {
        ...
        if (currentImage && currentImage.src) {
            ...
            try {
                ...
                // Decode the pathname first to prevent double-encoding of special characters
...
document.addEventListener('DOMContentLoaded', () => {
    // ===== NOTIFICATION TOGGLE =====
    const bellIcon = document.querySelector('.notification .fa-bell');
    ...
    if (bellIcon && notiModal) {
        ...
        document.addEventListener('click', (e) => {
            if (
                notiModal.classList.contains('show') &&
...
// Modal comment 
    ...
    if (commentIcon && imageCommentPreview) {
        commentIcon.addEventListener('click', () => {
            const isHidden = imageCommentPreview.style.display === "none" || imageCommentPreview === '';
            if(isHidden) {
                imageCommentPreview.style.display = 'block';
                commentIcon.classList.replace('fa-regular', 'fa-solid');
            } else {
                  imageCommentPreview.style.display = 'none';
                  commentIcon.classList.replace('fa-solid', 'fa-regular');
            }
        });
    }
